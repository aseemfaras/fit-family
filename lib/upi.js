export function buildUPI({ amount, orderId, paymentToken = '' }) {
    ...
    // Build notes field with order ID and payment token
    let notes = `OrderID: ${orderId}`;
    if (paymentToken) {
        notes += ` | Token: ${paymentToken}`;
    }
    
    // Use CONFIG.BUSINESS_UPI_ID and BUSINESS_NAME from config
    // tn parameter is for transaction note/remarks
    const upiURI = `upi://pay?pa=${encodeURIComponent(CONFIG.BUSINESS_UPI_ID)}&pn=${encodeURIComponent(CONFIG.BUSINESS_NAME)}&am=${am}&cu=INR&tn=${encodeURIComponent(notes)}`;
    return upiURI;
}
